...
if (data.direction.x=='right' && ship.position.x < 20) {
...
if (data.direction.x=='left' && ship.position.x > -20) {
        ship.position.x=ship.position.x-xSpeed*(data.force*2)*(revcounter/400);
              if (!controls.enabled) { camera.position.x = ship.position.x;}
        if (ship.rotation.z> -0.2) {
          ship.rotation.z = -data.force/40;
...
ship.rotation.y = 0;
ship.rotation.x = 0;
mtrail.material.opacity = revcounter/300;
});// on end
...
var accbtn = document.getElementById("accelerator");
...
var geometry = new THREE.CylinderBufferGeometry( 0.5, 0.5, xwd, 8, 1, true );
var trmaterial = new THREE.MeshBasicMaterial( { 
  color: 0xffffff, transparent : true, opacity : 0, map:grad
 } );
mtrail = new THREE.Mesh( geometry, trmaterial );
mtrail.name='trail';
mtrail.position.set(0,0,xwd/2)
...
//
      ...
      // ccube = new THREE.Mesh( cgeometry, cmaterial );
...
function render() {
  ...
  // mtrail.material.opacity = 0.0001+revcounter/800;
  hudupdate();
  TWEEN.update();
...
new TWEEN.Tween( ccube.position ).to( {x: 0, y: 0, z: -100}, 500 )
.easing( TWEEN.Easing.Quadratic.EaseInOut)
.start();
